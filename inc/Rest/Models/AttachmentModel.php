...
class AttachmentModel {
	...
	protected function base_fields( int $attachment_id, Context $context, ?int $parent_id, string $field_key ): array {
		...
		$data['src'] = $context->relative_attachment_urls
			? apply_filters( 'blank_rest_relative_attachment_url', $meta['file'], $url )
			: $url;

		if ( $context->with_acf ) {
			$data['acf'] = apply_filters( 'blank_rest_attachment_acf', $attachment_id );
		}
