...
class PostModel {
	...
	protected function base_fields( WP_Post $post, ModelContext $context ): array {
		...
		if ( $context->with_acf ) {
			$data['acf'] = apply_filters( 'blank_rest_post_acf', $data['id'] );
		}

		if ( $context->relative_urls ) {
			$data['link'] = apply_filters( 'blank_relative_url_enabled', $data['link'] );
		}

		if ( $context->embed_attachments ) {
			$data['featured_media'] = apply_filters( 'blank_embed_featured_attachment', $data['featured_media'], $post );
			$data['attachements']   = apply_filters( 'blank_embed_post_attachments', $post );
		}

		if ( $context->embed_author ) {
			$data['author'] = apply_filters( 'blank_embed_author', $post );
		}

		if ( $context->embed_terms ) {
			$data['terms'] = apply_filters( 'blank_embed_terms', $post );
		}
