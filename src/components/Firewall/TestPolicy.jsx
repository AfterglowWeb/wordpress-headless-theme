...
export default function TestPolicy( { route, method, hasChildren = false } ) {
	...
	const runTest = async () => {
		...
		try {
			...
			if ( result?.success ) {
				...
				setError( result?.data?.message || __( 'Test failed', 'blank' ) );
			}
		} catch ( err ) {
			setError( err.message || __( 'Network error', 'blank' ) );
		} finally {
			setLoading( false );
		}
	};
	...
	const getTestLabel = ( testName ) => {
		switch ( testName ) {
			case 'auth':
				return __( 'Authentication', 'blank' );
			case 'rate_limit':
				return __( 'Rate Limit', 'blank' );
			case 'disabled':
				return __( 'Disabled', 'blank' );
			default:
				return testName;
		}
	};
	...
	const renderPolicyInfo = ( policy ) => {
		return (
			<Stack direction="row" spacing={ 1 } flexWrap="wrap" sx={ { mb: 1 } }>
				<Chip
					label={ policy.state ? __( 'Enabled', 'blank' ) : __( 'Disabled', 'blank' ) }
					size="small"
					color={ policy.state ? 'success' : 'error' }
					variant="outlined"
				/>
				<Chip
					label={ policy.protect ? __( 'Protected', 'blank' ) : __( 'Public', 'blank' ) }
					size="small"
		...
		return (
			<Stack spacing={ 1 } sx={ { mt: 2 } }>
				<Typography variant="subtitle2" fontWeight={ 600 }>
					{ __( 'Results', 'blank' ) } ({ results.length })
				</Typography>
		...
		<>
			...
			>
							...
							<Stack spacing={ 0.5 }>
								{ hasChildren && (
									<FormControlLabel
										...
										label={ __( 'Include sub-routes', 'blank' ) }
									/>
								) }
								...
								<FormControlLabel
									...
									label={ __( 'Test authentication', 'blank' ) }
								/>
								...
								<FormControlLabel
									...
									label={ __( 'Test rate limiting', 'blank' ) }
								/>
								...
								<FormControlLabel
									...
									label={ __( 'Test disabled state', 'blank' ) }
								/>
							</Stack>
						</Box>
				...
				<DialogActions>
					...
					>
						{ loading ? __( 'Running...', 'blank' ) : __( 'Run Test', 'blank' ) }
					</Button>
				</DialogActions>
			</Dialog>
		</>
