...
export default function IpFilter() {
	...
	const handleSave = async () => {
		openDialog( {
			...
			content: __( 'Are you sure you want to save the IP filter settings?', 'blank' ),
			onConfirm: async () => {
				updateDialog( {
					type: DIALOG_TYPES.LOADING,
					title: __( 'Saving', 'blank' ),
					content: __( 'Saving IP filter settings...', 'blank' ),
				} );
				...
				try {
					...
					if ( result?.success ) {
						updateDialog( {
							...
							content: __( 'IP filter settings saved successfully!', 'blank' ),
							autoClose: 2000,
						} );
					} else {
						updateDialog( {
							...
							content: result?.data?.message || __( 'Failed to save settings', 'blank' ),
						} );
					}
				} catch ( error ) {
					updateDialog( {
	...
	const handleAddIp = () => {
		...
		if ( ! trimmed ) {
			setIpError( __( 'Please enter an IP address', 'blank' ) );
			return;
		}

		if ( ! isValidIpOrCidr( trimmed ) ) {
			setIpError( __( 'Invalid IP address or CIDR range', 'blank' ) );
			return;
		}
		...
		if ( currentList.includes( trimmed ) ) {
			setIpError( __( 'This IP is already in the list', 'blank' ) );
			return;
		}
	...
	if ( loading ) {
		return (
			<Box sx={ { py: 2 } }>
				<Typography color="text.secondary">{ __( 'Loading...', 'blank' ) }</Typography>
			</Box>
		);
	}
		...
		<Stack spacing={ 3 }>
			...
			<Stack direction={{ xs: 'column', sm: 'row' }} spacing={ 3 }>
				<FormControl>
					<FormControlLabel
						...
						label={ __( 'Enable IP Filtering', 'blank' ) }
					/>
					<FormHelperText>
						{ __( 'Block or allow requests based on IP address', 'blank' ) }
					</FormHelperText>
				</FormControl>

				<FormControl sx={ { minWidth: 200 } }>
					<InputLabel id="ip-mode-label">{ __( 'Filter Mode', 'blank' ) }</InputLabel>
					<Select
						labelId="ip-mode-label"
						value={ settings.mode }
						onChange={ handleModeChange }
						label={ __( 'Filter Mode', 'blank' ) }
						size="small"
						disabled={ ! settings.enabled }
					>
						<MenuItem value="blacklist">{ __( 'Blacklist', 'blank' ) }</MenuItem>
						<MenuItem value="whitelist">{ __( 'Whitelist', 'blank' ) }</MenuItem>
					</Select>
					<FormHelperText>
						{ settings.mode === 'blacklist'
							? __( 'Block listed IPs, allow all others', 'blank' )
							: __( 'Allow only listed IPs, block all others', 'blank' ) }
					</FormHelperText>
				</FormControl>
			</Stack>
			...
			{ settings.mode === 'whitelist' && settings.enabled && settings.whitelist.length === 0 && (
				<Alert severity="warning">
					{ __( 'Warning: Whitelist mode is enabled but the list is empty. No IPs will be blocked until you add entries.', 'blank' ) }
				</Alert>
			) }
			...
			<Box>
				<Typography variant="subtitle2" sx={ { mb: 1 } }>
					{ settings.mode === 'whitelist'
						? __( 'Whitelisted IPs', 'blank' )
						: __( 'Blacklisted IPs', 'blank' ) }
					<Chip
				...
				<Stack direction="row" spacing={ 1 } sx={ { mb: 2 } }>
					<TextField
						...
						size="small"
						error={ !! ipError }
						helperText={ ipError || __( 'Enter IP address or CIDR range (e.g., 192.168.1.0/24)', 'blank' ) }
						disabled={ ! settings.enabled }
								...
								<ListItemText
									primary={
										<Stack direction="row" spacing={ 1 } alignItems="center">
											...
											{ ip === clientIp && (
												<Chip label={ __( 'Your IP', 'blank' ) } size="small" color="warning" />
											) }
										</Stack>
									}
								/>
			...
			{ settings.mode === 'blacklist' && settings.whitelist.length > 0 && (
				<Box sx={ { opacity: 0.6 } }>
					<Typography variant="caption" color="text.secondary">
						{ __( 'Whitelist (inactive):', 'blank' ) } { settings.whitelist.join( ', ' ) }
					</Typography>
				</Box>
			) }
			{ settings.mode === 'whitelist' && settings.blacklist.length > 0 && (
				<Box sx={ { opacity: 0.6 } }>
					<Typography variant="caption" color="text.secondary">
						{ __( 'Blacklist (inactive):', 'blank' ) } { settings.blacklist.join( ', ' ) }
					</Typography>
				</Box>
			) }
		</Stack>
